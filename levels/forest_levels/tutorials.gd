...
func _ready() -> void:
	moviment_text.text = "Aperte " + KEYBIND_DEFAULT.move_left_key.as_text_physical_keycode() + " para ir para esquerda\nAperte " + KEYBIND_DEFAULT.move_right_key.as_text_physical_keycode() + " para ir para direita"
	jump_text.text = "Aperte " + KEYBIND_DEFAULT.jump_key.as_text_physical_keycode() + " para pular"
